...
const items = [
  { desc: "Design de identidade visual", qty: 1, price: 2800, addAt: 40 },
  { desc: "Cardápio impresso (3 versões)", qty: 1, price: 1200, addAt: 95 },
  { desc: "Posts redes sociais (mensal)", qty: 3, price: 800, addAt: 155 },
];
...
export const NewProposal: React.FC = () => {
  ...
  const titleText = "Identidade visual + cardápio".slice(0, Math.max(0, Math.floor((frame - 10) / 1.4)));
  ...
  return (
    <AbsoluteFill>
      <AppFrame active="Propostas" pageTitle="Nova proposta">
        <div style={{ display: "flex", gap: 20, height: "100%" }}>
          ...
          >
            <Field
              label="Título"
              value={titleText}
              active={frame > 10 && frame < 40}
            />
            <div style={{ display: "flex", gap: 14 }}>
              <Field label="Cliente" value="Padaria Estrela" wide />
              <Field label="Validade" value="30 dias" />
            </div>

            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
              <div style={{ fontFamily: fontDisplay, fontSize: 16, fontWeight: 600, color: colors.text }}>Itens</div>
              <div
                style={{
                  padding: "8px 14px",
              ...
              {items.map((it, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ color: colors.muted }}>R$ {it.price.toLocaleString("pt-BR")}</div>
                    <div style={{ fontWeight: 600, textAlign: "right" }}>R$ {(it.qty * it.price).toLocaleString("pt-BR")}</div>
                  </div>
                );
              })}
            </div>
          ...
          >
            <div style={{ fontFamily: fontDisplay, fontSize: 18, fontWeight: 600, color: colors.text }}>Resumo</div>
            <Row label="Subtotal" value={`R$ ${Math.round(total).toLocaleString("pt-BR")}`} />
            <Row label="Desconto" value="R$ 0" />
      ...
      <Cursor
        ...
        click={[40, 95, 155]}
      />
      <Caption text="Crie propostas com cálculo automático." from={10} />
    </AbsoluteFill>
  );
};
