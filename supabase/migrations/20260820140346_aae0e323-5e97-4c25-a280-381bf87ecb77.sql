...
CREATE OR REPLACE FUNCTION public.get_admin_referrals()
 RETURNS TABLE(id uuid, referral_code text, status text, discount_percent integer, created_at timestamp with time zone, converted_at timestamp with time zone, paid_at timestamp with time zone, reward_granted_at timestamp with time zone, referrer_full_name text, referrer_email text, referred_full_name text, referred_email text)
 LANGUAGE plpgsql
